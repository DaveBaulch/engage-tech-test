...
Vue.directive("animate", {
  ...
  bind(el, binding) {
    ...
    if (setup.length) {
      setup.forEach((setup) => {
        ...
        console.log(setups);
        console.log(setup);
        gsap.set(el, { ...setups[setup], duration: 0, delay: 0 });
  ...
  inserted(el, binding) {
    ...
    //let str = JSON.stringify(animation);
    let str = JSON.stringify(animation, null, 4); // (Optional) beautiful indented output.
