<template>
  <router-link v-if="type === 'router-link'" :to="{ name: 'Home', params: { id: itemData.id } }" :class="style"
    >{{
    <figure v-if="icon">
  ...
  >
  <a v-else-if="type === 'link'" :href="to" :class="style">
    <figure v-if="icon">
      <img :src="icon" alt="alt" />
    </figure>
    <slot><span v-if="text" v-html="text"></span></slot
  ></a>
</template>

  <router-link :to="{ name: 'Home', params: { id: itemData.id } }">
    <li>
