...
const itemModule = {
  ...
  actions: {},
  getters: {
    getCategoryItems: (state) => (itemData) => {
      return state.items.filter((item) => item.categoryId === itemData.id);
    },
    getCategoryItemById: (state) => (itemData) => {
      return state.item.id === itemData.id;
    }
  }
};
